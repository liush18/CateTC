							...
							const nav_t *nav, double *dtrp, double *var, cate_t *cate)
{
	...
	if (opt->tropopt == TROPOPT_EST || opt->tropopt == TROPOPT_ESTG) {
		matcpy(trp, x + GIT(&cate->opt), opt->tropopt == TROPOPT_EST ? 1 : 3, 1);
		*dtrp = trop_model_prec(time, pos, azel, trp, dtdx, var);
		return 1;
	}
	if (opt->tropopt == TROPOPT_ZTD) {
...
extern void Cate_satantpcv(const double *rs, const double *rr, const pcv_t *pcv,
	double *dant)
{
	satantpcv(rs, rr, pcv, dant);
}

extern int Cate_model_phw(gtime_t time, int sat, const char *type, int opt,
	const double *rs, const double *rr, double *phw)
{
	return model_phw(time, sat, type, opt, rs, rr, phw);
}

extern void Cate_corr_meas(const obsd_t *obs, const nav_t *nav, const double *azel,
	const prcopt_t *opt, const double *dantr,
	const double *dants, double phw, double *L, double *P,
	double *Lc, double *Pc)
{
	corr_meas(obs, nav, azel, opt, dantr, dants, phw, L, P, Lc, Pc);
}
