...
static imu_t imuraw = { 0 };
static imu_t imualg = { 0 };
static imu_t imusol = { 0 };
static solvd_t *solvf;
static solvd_t *solvb;
static int isolvf = 0;
static int isolvb = 0;

static void Free_IMU(imu_t *imu)
{
	imu->n = 0; imu->nmax = 0;
	free(imu->data); imu->data = NULL;
}

static void Free_INS()
{
	follow(2, "Free_INS...\n");
	Free_IMU(&imuraw); Free_IMU(&imualg); Free_IMU(&imusol);
}

static void Init_Cate(cate_t *cate, const prcopt_t *popt, const solopt_t *sopt,
	const cateopt_t *opt, const calcuopt_t *copt)
{
	int i;
	solvd_t solv0 = { 0 };
	csat_t csat0 = { 0 };
	...
	cate->nx = (opt->coupled_type == PMODE_TTCP) ? ttcpnx(opt, popt) : lscpnx(opt);
	cate->tt = 0.0;
	cate->xx = zeros(cate->nx, 1);
	cate->P = zeros(cate->nx, cate->nx);

	for (i = 0; i < MAXSAT; i++)	cate->csat[i] = csat0;
	cate->opt = *opt;
	cate->solv = solv0;
	if (popt) { cate->popt = *popt; }
	if (sopt) { cate->sopt = *sopt; }
	if (copt) { cate->copt = *copt; }

	if (popt->soltype == 1) {
		matcpy(cate->solv.re, opt->bre, 3, 1);
		matcpy(cate->solv.ve, opt->bve, 3, 1);
	}
	else {
		matcpy(cate->solv.re, opt->fre, 3, 1);
		matcpy(cate->solv.ve, opt->fve, 3, 1);
	}

}

static void Free_Cate(cate_t *cate)
{
	follow(2, "Free_Cate...\n");

	cate->nx = 0;// cate->vsn = 0;
	cate->tt = 0.0;
	free(cate->xx); cate->xx = NULL;
	free(cate->P); cate->P = NULL;
}

/* combine forward/backward solutions of tightly coupled and output results */
static void comb_solvd()
{
	solvd_t solvs = { {0} };
	...
	for (i = 0, j = isolvb - 1; i < isolvf&&j >= 0; i++, j--) {
		...
		if ((tt = timediff(solvf[i].time, solvb[j].time)) < -DTTOL) {
			solvs = solvf[i];
			j++;
		}
		else if (tt > DTTOL) {
			...
			/* smoother for position solution */
			for (k = 0; k < 3; k++) {
				Qf[k + k * 3] = solvf[i].qr[k];
				Qb[k + k * 3] = solvb[j].qr[k];
			}
			Qf[1] = Qf[3] = solvf[i].qr[3];
			Qf[5] = Qf[7] = solvf[i].qr[4];
			Qf[2] = Qf[6] = solvf[i].qr[5];
			Qb[1] = Qb[3] = solvb[j].qr[3];
			Qb[5] = Qb[7] = solvb[j].qr[4];
			Qb[2] = Qb[6] = solvb[j].qr[5];
			if (smoother(solvf[i].re, Qf, solvb[j].re, Qb, 3, solvs.re, Qs)) continue;
			solvs.qr[0] = (float)Qs[0];
			solvs.qr[1] = (float)Qs[4];
			solvs.qr[2] = (float)Qs[8];
			solvs.qr[3] = (float)Qs[1];
			solvs.qr[4] = (float)Qs[5];
			solvs.qr[5] = (float)Qs[2];

			/* smoother for velocity solution */
			for (k = 0; k < 3; k++) {
				Qf[k + k * 3] = solvf[i].qv[k];
				Qb[k + k * 3] = solvb[j].qv[k];
			}
			Qf[1] = Qf[3] = solvf[i].qv[3];
			Qf[5] = Qf[7] = solvf[i].qv[4];
			Qf[2] = Qf[6] = solvf[i].qv[5];
			Qb[1] = Qb[3] = solvb[j].qv[3];
			Qb[5] = Qb[7] = solvb[j].qv[4];
			Qb[2] = Qb[6] = solvb[j].qv[5];
			if (smoother(solvf[i].ve, Qf, solvb[j].ve, Qb, 3, solvs.ve, Qs)) continue;
			solvs.qv[0] = (float)Qs[0];
			solvs.qv[1] = (float)Qs[4];
			solvs.qv[2] = (float)Qs[8];
			solvs.qv[3] = (float)Qs[1];
			solvs.qv[4] = (float)Qs[5];
			solvs.qv[5] = (float)Qs[2];

			/* smoother for attitude solution */
			for (k = 0; k < 3; k++) {
				Qf[k + k * 3] = solvf[i].qa[k];
				Qb[k + k * 3] = solvb[j].qa[k];
			}
			Qf[1] = Qf[3] = solvf[i].qa[3];
			Qf[5] = Qf[7] = solvf[i].qa[4];
			Qf[2] = Qf[6] = solvf[i].qa[5];
			Qb[1] = Qb[3] = solvb[j].qa[3];
			Qb[5] = Qb[7] = solvb[j].qa[4];
			Qb[2] = Qb[6] = solvb[j].qa[5];
			if (smoother(solvf[i].att, Qf, solvb[j].att, Qb, 3, solvs.att, Qs)) continue;
			solvs.qa[0] = (float)Qs[0];
			solvs.qa[1] = (float)Qs[4];
			solvs.qa[2] = (float)Qs[8];
			solvs.qa[3] = (float)Qs[1];
			solvs.qa[4] = (float)Qs[5];
			solvs.qa[5] = (float)Qs[2];
		}
...
static void proc_tc(const prcopt_t *popt, const solopt_t *sopt, const cateopt_t *opt, double *Cb2e, const int mode)
{
	rtk_t rtk;
	cate_t cate;
	obsd_t obs[MAXOBS * 2];
	imud_t imu;
	gtime_t PreviousEpTime = { 0 };
	calcuopt_t copt;
	double tHz = 1.0 / opt->Hz, dt, k;
	char msg[128] = "";
	int nobs, nobsv1, nobsv2, imun, state = 1, nsolv = 0;

	follow(2, "ttcp_proc...\n");

	CycSlip_Thres(opt->si, &copt.gft, &copt.mwt);
	rtkinit(&rtk, popt);
	Init_Cate(&cate, popt, sopt, opt, &copt);

	if (revs) {
		imun = imusol.n - 1; k = -1.0;
	}
	else {
		imun = 0; k = 1.0;
	}
	while (InputIMU(&imusol, &imu, revs, &imun, cate.solv.stat, 0)) {

		cate.solv.stat = CSOL_NONE;

		while (state) {
			if ((nobs = inputobs(obs, rtk.sol.stat, popt)) < 0) continue;
			if (!revs && timediff(obs->time, imu.time) < 0.0) continue;
			if (revs && timediff(obs->time, imu.time) > 0.0) continue;
			if ((nobsv1 = ObsMeasSPP_Detec(popt, obs, nobs)) <= 0) continue;
			if (!pntpos(obs, nobsv1, &navs, popt, &rtk.sol, NULL, rtk.ssat, msg)) {
				follow(1, "pntpos error : (%s)\n", msg);
			}
			if ((nobsv2 = ObsMeasPPP_Detec(obs, nobs)) < 1) continue;
			state = 0;
			break;
		}
		
		if (opt->corr_imu) {
			Corr_IMUd(&imu, &cate.solv, opt->corr_imu_k);
		}

		dt = fabs(timediff(obs->time, imu.time));  /* timediff前向滤波为正，后向为负 */
		if (dt < 1E-6) {
			if (PreviousEpTime.time != 0) {
				cate.tt = timediff(obs->time, PreviousEpTime);
			}
			cate.solv.time = obs->time;
			FinsPos(&cate, &imu, Cb2e, k*tHz, 0);
			TC_Pos(&cate, Cb2e, imu.acce, imu.gyro, &rtk, obs, nobsv2, &navs);
			state = 1;
			PreviousEpTime = obs->time;
			cOutSolStat(&cate, &rtk);
		}
		else if (dt < tHz) {
			if (PreviousEpTime.time != 0) {
				cate.tt = timediff(obs->time, PreviousEpTime);
		...
		if (state && dt > 1E-6 && dt < tHz) {
			cate.solv.time = imu.time;
			FinsPos(&cate, &imu, Cb2e, k*(tHz - dt), 0);
		}
	}
...
static int exec_tc(const prcopt_t *popt, const solopt_t *sopt, const cateopt_t *opt, double *Cb2e)
{
	
	iobsu = iobsr = isbs = ilex = revs = aborts = 0;
	if (popt->soltype == 0) {
		proc_tc(popt, sopt, opt, Cb2e, 0);
	}
		...
		if (solvf&&solvb) {
			...
			follow(1, "ttcp_exec memory allocation error\n");
		}
...
static int exec_gnss(gtime_t ts, gtime_t te, double ti, prcopt_t *popt, solopt_t *sopt, 
					const filopt_t *fopt, char **infile, int *index, int n)
{
	prcopt_t popt_ = *popt;
	char path[1024], *ext;
	int stat = 0;
	...
	/* open processing session */
	if (!openses(popt, sopt, fopt, &navs, &pcvss, &pcvsr)) return 0;
...
static void opt2Cb2e(const cateopt_t *opt, double *Cb2e, int soltype)
{
	double att[3], re[3], pos[3];
	if (soltype == 1) {
		matcpy(re, opt->bre, 3, 1);
		matcpy(att, opt->batt, 3, 1);
	}
	else {
		matcpy(re, opt->fre, 3, 1);
		matcpy(att, opt->fatt, 3, 1);
	}

	ecef2pos(re, pos);
	Att_to_Cb2e(att, pos, Cb2e);
}

static int exec_ins(const cateopt_t *opt, filepath_t *flp, double *Cb2e, int soltype)
{

	if (!File_Open(opt, flp->solution)) { return 0; }

	/* alignment */
	if (opt->align_type == ALIGNATT) {
		opt2Cb2e(opt, Cb2e, soltype);
		if (*flp->insolv) {
			if (!Read_IMU(flp->insolv, &imusol, opt)) { return 0; }
		}
		else {
			if (!Read_IMU(flp->inimu, &imuraw, opt) || !Divide_IMU(&imuraw, opt, &imualg, &imusol)) { return 0; }
		}
	}
	else if (opt->align_type == ALIGNCOR) {
		if (*flp->inalign && *flp->insolv) {
			if (!Read_IMU(flp->inalign, &imualg, opt) || !Read_IMU(flp->insolv, &imusol, opt)) { return 0; }
		}
		else {
			if (!Read_IMU(flp->inimu, &imuraw, opt) || !Divide_IMU(&imuraw, opt, &imualg, &imusol)) { return 0; }
		}
		Init_Align(&imualg, opt->fre, opt->Hz, Cb2e);
	}

	if (!Out_Header(flp->solution)) {
		follow(1, "open error: %s\n", flp->solution);
		return 0;
	}
					...
					int *index, int n)
{
	...
	if (!exec_ins(opt, flp, Cb2e, popt->soltype)) {
		follow(1, "exec_ins error\n");
		return 0;
	}
	exec_gnss(opt->ts, opt->te, opt->ti, popt, sopt, fopt, infile, index, n);
	exec_tc(popt, sopt, opt, Cb2e);
